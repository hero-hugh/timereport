...
describe('ProjectService', () => {
	...
	describe('getProjects', () => {
		it('should return only active projects by default', async () => {
			...
			const inactiveProject = await projectService.createProject(testUserId, {
				name: 'Inactive Project',
				startDate: '2024-01-01',
			})

			await projectService.updateProject(inactiveProject.id, testUserId, {
				isActive: false,
			})
		...
		it('should return all projects when includeInactive is true', async () => {
			...
			const projects = await projectService.getProjects(testUserId, true)
	...
	describe('getProject', () => {
		...
		it('should return null for other users project', async () => {
			...
			const project = await projectService.createProject(otherUser.id, {
				name: 'Other Project',
				startDate: '2024-01-01',
			})

			const result = await projectService.getProject(project.id, testUserId)
	...
	describe('deleteProject', () => {
		it('should delete project', async () => {
			...
			const result = await projectService.deleteProject(project.id, testUserId)
